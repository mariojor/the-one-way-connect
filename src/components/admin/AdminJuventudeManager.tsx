...
const AdminJuventudeManager = () => {
  const [conteudos, setConteudos] = useState<ConteudoJuventude[]>([]);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Tem certeza que deseja excluir este conteúdo?")) {
      try {
        const response = await fetch(`http://localhost:3001/api/juventude/${id}`, {
          method: "DELETE",
        });
                ...
                <TableRow key={conteudo.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      <Link to={`/admin/juventude/editar/${conteudo.id}`}>
...
export default AdminJuventudeManager;
