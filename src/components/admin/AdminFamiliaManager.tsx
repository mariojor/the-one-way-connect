...
const AdminFamiliaManager = () => {
  ...
  const fetchConteudos = async () => {
    try {
      setLoading(true);
      const response = await fetch("http://localhost:3001/api/familia");
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Tem certeza que deseja excluir este conteúdo?")) {
      try {
        const response = await fetch(`http://localhost:3001/api/familia/${id}`, {
          method: "DELETE",
        });
                ...
                <TableRow key={conteudo.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      <Link to={`/admin/familia/editar/${conteudo.id}`}>
...
export default AdminFamiliaManager;
